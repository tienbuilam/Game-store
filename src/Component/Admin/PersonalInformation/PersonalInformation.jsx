...
function AdminPersonalInfo() {
    ...
    const calculateRemainHeight = () => {
        if (isRefValid(bigDiv) && isRefValid(div1Height) && isRefValid(div2Height) && isRefValid(buttonHeight)) {
            const height =
                bigDiv.current.offsetHeight - div1Height.current.offsetHeight - buttonHeight.current.offsetHeight - 50;
            div2Height.current.style.height = `calc(${height}px)`;
        }
    };

    useEffect(() => {
        axios.get(`http://${domain}/admin/info`, { withCredentials: true }).then((res) => {
            ...
            setadmin({
                ...
                image: res.data.image === null ? person : `http://${domain}/model/data/admins/${res.data.image}`,
                dob: res.data.dob,
            });
            if (isRefValid(profileImg))
                profileImg.current.src =
                    res.data.image === null ? person : `http://${domain}/model/data/admins/${res.data.image}`;
        });
    ...
    const changeInfo = () => {
        if (isRefValid(admin_email)) admin_email.current.style.display = 'none';
        if (isRefValid(admin_phone)) admin_phone.current.style.display = 'none';
        if (isRefValid(admin_dob)) admin_dob.current.style.display = 'none';
        if (isRefValid(edit)) edit.current.style.display = 'none';
        if (isRefValid(admin_name)) admin_name.current.style.display = 'none';
        if (isRefValid(admin_address)) admin_address.current.style.display = 'none';
    ...
    const cancelUpdate = () => {
        if (isRefValid(admin_email)) admin_email.current.style.display = 'inline';
        if (isRefValid(admin_phone)) admin_phone.current.style.display = 'inline';
        if (isRefValid(admin_dob)) admin_dob.current.style.display = 'inline';
        if (isRefValid(edit)) edit.current.style.display = 'inline';
        if (isRefValid(admin_name)) admin_name.current.style.display = 'inline';
        if (isRefValid(admin_address)) admin_address.current.style.display = 'inline';

        if (isRefValid(edit)) edit.current.style.display = 'inline-block';

        if (isRefValid(update)) update.current.style.display = 'none';
        if (isRefValid(admin_email_input)) admin_email_input.current.style.display = 'none';
        if (isRefValid(admin_phone_input)) admin_phone_input.current.style.display = 'none';
        if (isRefValid(admin_dob_input)) admin_dob_input.current.style.display = 'none';
        if (isRefValid(admin_password_input)) admin_password_input.current.style.display = 'none';
        if (isRefValid(admin_repassword_input)) admin_repassword_input.current.style.display = 'none';
        if (isRefValid(admin_name_input)) admin_name_input.current.style.display = 'none';
        if (isRefValid(admin_address_input)) admin_address_input.current.style.display = 'none';
    ...
    const confirmChange = () => {
        if (isRefValid(admin_email_input) && admin_email_input.current.value === '') {
            ...
            isRefValid(admin_dob_input) &&
            admin_dob_input.current.value !== '' &&
            new Date(admin_dob_input.current.value) > new Date()
        ) {
            if (isRefValid(pop_up_5)) pop_up_5.current.style.display = 'flex';
            ...
            formData.append(
                'address',
                isRefValid(admin_address_input)
                    ? admin_address_input.current.value === ''
                        ? null
                        : admin_address_input.current.value
                    : null,
            );
    ...
    return (
        <div className="d-flex align-items-center w-100 h-100">
            <div className={`flex-grow-1 w-100 mt-3 overflow-auto mb-3`} ref={bigDiv}>
                <div
                ...
                >
                    <div className={`d-flex flex-column ${styles.containDiv}`}>
                        ...
                        <label
                            className={`btn btn-sm btn-light border border-dark mt-3 mx-auto ${styles.browse}`}
                            ref={image_input}
                        >
                            <input
                                type="file"
                                ...
                                onChange={(e) => {
                                    if (e.target.files.length === 0) {
                                        setImage(null);
                                        if (isRefValid(profileImg)) profileImg.current.src = admin.image;
                                    } else {
                                        ...
                                        reader.onload = () => {
                                            ...
                                            if (isRefValid(profileImg)) profileImg.current.src = url;
                                        };
                                    }
                                }}
                                accept=".jpg,.jpeg,.png"
                    ...
                    >
                        ...
                        <div className="text-center w-100" style={{ marginBottom: '16px' }}>
                            Date of birth: &nbsp;
                        ...
                        <div
                            ...
                            ref={update}
                        >
                            <button className={`btn btn-sm btn-danger d-block`} onClick={cancelUpdate}>
                                Cancel
                            </button>
                            <button
                                className={`btn btn-sm btn-primary mx-3 d-block`}
                                onClick={() => {
