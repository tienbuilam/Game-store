...
function CustomerDetail() {
    ...
    useEffect(() => {
        ...
        axios.post(`http://${domain}/admin/customer/detail`, formData).then((res) => {
            ...
            if (isRefValid(discount_input)) discount_input.current.value = res.data.membership_discount;
        });
    ...
    const getHistory = () => {
        ...
        formData.append('id', id);
        axios
            .post(`http://${domain}/admin/customer/detail/history`, formData)
            .then((res) => {
    ...
    const changeInfo = () => {
        if (isRefValid(rank)) rank.current.style.display = 'none';
        if (isRefValid(discount)) discount.current.style.display = 'none';
    ...
    const cancelUpdate = () => {
        if (isRefValid(rank)) rank.current.style.display = 'inline';
        if (isRefValid(discount)) discount.current.style.display = 'inline';
        ...
        if (isRefValid(discount_input)) discount_input.current.style.display = 'none';
        ...
        if (isRefValid(select_menu)) select_menu.current.style.display = 'none';
    };
    ...
    const confirmChange = () => {
        if (isRefValid(customer_email_input) && customer_email_input.current.value === '') {
            ...
            isRefValid(select_menu) &&
            select_menu.current.value === 'Special' &&
            isRefValid(discount_input) &&
            (discount_input.current.value > 5 || discount_input.current.value < 0)
        ) {
            ...
            formData.append(
                'phone',
                customer_phone_input.current.value === '' ? null : customer_phone_input.current.value,
            );
            axios
    ...
    return (
        <div className="d-flex flex-column align-items-center w-100 h-100">
            <div className="d-flex align-items-center w-100" style={{ height: '50px' }}>
                <BiTrash
                    className={`ms-auto ms-md-3 me-md-0 me-3 ${styles.trash}`}
                    onClick={() => {
