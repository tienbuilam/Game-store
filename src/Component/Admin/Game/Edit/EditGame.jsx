...
const Category = (props) => {
    const change = (event) => {
        if (!event.target.checked) props.setRemovedTag((prevTags) => [...prevTags, event.target.value]);
        else props.setTag((prevTags) => [...prevTags, event.target.value]);
    };
    ...
    return (
        <div className={`d-flex align-items-center col-${12 / props.length} mb-3`}>
            <input
                ...
                type="checkbox"
                ref={(el) => (props.checkbox.current[props.index] = el)}
            ...
            if (props.data[i] !== undefined)
                temp.push(
                    <Category
                        ...
                        key={i}
                        category={props.data[i].type}
                        setRemovedTag={props.setRemovedTag}
                        setTag={props.setTag}
                        checkbox={props.checkbox}
                        index={props.elementPerLine * props.i + i}
                    />,
                );
        }
        if (isRefValid(root)) root.current.render(<>{temp}</>);
    });
...
const EditGame = () => {
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((res) => {
                ...
                if (isRefValid(nameRef)) nameRef.current.value = res.data.name;

                setPrice(res.data.price);
                if (isRefValid(priceRef)) priceRef.current.value = res.data.price;

                setDiscount(res.data.discount);
                if (isRefValid(discountRef)) discountRef.current.value = res.data.discount;

                setOldPic1(
                    res.data.picture_1 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_1}`,
                );
                setPic1(res.data.picture_1 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_1}`);

                setOldPic2(
                    res.data.picture_2 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_2}`,
                );
                setPic2(res.data.picture_2 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_2}`);

                setOldPic3(
                    res.data.picture_3 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_3}`,
                );
                setPic3(res.data.picture_3 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_3}`);

                setOldPic4(
                    res.data.picture_4 === null ? '' : `http://${domain}/model/data/games/${res.data.picture_4}`,
                );
        ...
        if (isRefNotValid(root) && isRefValid(renderTarget)) root.current = ReactDOM.createRoot(renderTarget.current);
        let elementPerLine;
        if (window.innerWidth < 375) elementPerLine = 1;
        else if (window.innerWidth < 992) elementPerLine = 2;
        else if (window.innerWidth < 2400) elementPerLine = 3;
        else elementPerLine = 4;
        axios
            .get(`http://${domain}/admin/game/categories`)
            .then((res) => {
                    ...
                    if (elementPerLine === 1)
                        ...
                        temp.push(
                            <Line
                                ...
                                elementPerLine={elementPerLine}
                                i={i}
                                data={[res.data[i * 2], res.data[i * 2 + 1]]}
                                checkbox={checkbox}
                                tag={tag}
                                setTag={setTag}
                                setRemovedTag={setRemovedTag}
                        ...
                        temp.push(
                            <Line
                                ...
                                elementPerLine={elementPerLine}
                                i={i}
                                data={[res.data[i * 3], res.data[i * 3 + 1], res.data[i * 3 + 2]]}
                                checkbox={checkbox}
                                tag={tag}
                                setTag={setTag}
                                setRemovedTag={setRemovedTag}
                        ...
                        temp.push(
                            <Line
                                ...
                                elementPerLine={elementPerLine}
                                i={i}
                                data={[res.data[i * 4], res.data[i * 4 + 1], res.data[i * 4 + 2], res.data[i * 4 + 3]]}
                                checkbox={checkbox}
                                tag={tag}
                                setTag={setTag}
                                setRemovedTag={setRemovedTag}
    ...
    const loadPicture = (e, number) => {
        ...
        if (e.target.files.length === 0) {
            ...
            if (number === 1) setPic1(file);
            else if (number === 2) setPic2(file);
            else if (number === 3) setPic3(file);
            else setPic4(file);
            ...
            reader.onload = () => {
                ...
                if (number === 1 && isRefValid(img1)) img1.current.src = url;
                else if (number === 2 && isRefValid(img2)) img2.current.src = url;
                else if (number === 3 && isRefValid(img3)) img3.current.src = url;
                else img4.current.src = url;
            };
        }
    };
    ...
    const updateGame = async (event) => {
        ...
        formData.append('id', id);
        await axios
            .post(`http://${domain}/admin/game/update`, formData)
            .then((res) => {
                console.log(res);
            })
        ...
        await axios
            .post(`http://${domain}/admin/game/addCode`, formData1)
            .then((Response) => {
        ...
        await axios
            .post(`http://${domain}/admin/game/addTag`, formData2)
            .then((Response) => {
                console.log(Response);
            })
            .catch((error) => {
    ...
    const stopChoosing = () => {
        for (let i = 0; i < checkbox.current.length; i++) {
            ...
            else if (isRefValid(checkbox, i)) checkbox.current[i].checked = false;
        }
    ...
    return (
        <div className="w-100 h-100 d-flex flex-column align-items-center justify-content-center">
            <div
                className={`${styles.categories} position-absolute flex-column align-items-center`}
                ref={choose_categories}
            >
                <h1>Choose categories</h1>
                <div className={`w-100 flex-grow-1 mb-2 overflow-auto`} ref={renderTarget}></div>
                <div className="mt-auto mb-3">
                    <button className={`mx-3 btn btn-danger btn-sm`} onClick={stopChoosing}>
                        Cancel
                    </button>
                    <button
            ...
            <div
                className={`d-flex flex-column align-items-center justify-content-center w-100 ${styles.titleBoard} mb-2`}
            >
            ...
            <div className="flex-grow-1 w-100 overflow-auto d-flex flex-column mb-1">
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            if (e.target.value !== '') {
                                ...
                                timer = setTimeout(() => {
                                    setName(oldName);
                                    if (isRefValid(nameRef)) nameRef.current.val = oldName;
                                }, 2000);
                            }
                        }}
                    ></input>
                ...
                <div className={`d-xxl-flex w-100 mt-3`}>
                    <div className="d-flex flex-column flex-lg-row w-100">
                        <div
                            className={`d-flex flex-column justify-content-center align-items-center ${styles.image_block}`}
                        >
                ...
                <div
                    className={`d-flex flex-column flex-lg-row align-items-center align-items-lg-start justify-content-lg-around w-100 mt-lg-5 mb-lg-3`}
                >
                    ...
                    <div className="d-flex align-items-center mb-3 mb-lg-0 text-center">
                        ...
                        <input
                            ...
                            ref={discountRef}
                            onChange={(e) => {
                                if (e.target.value !== '') setDiscount(e.target.value);
                    ...
                    <div className="d-flex align-items-center mb-3 mb-lg-0 text-center">
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (isRefValid(choose_categories)) choose_categories.current.style.display = 'flex';
                            }}
                        >
                            Browse
                        </button>
                ...
                <div
                    className={`d-flex flex-column flex-lg-row align-items-center align-items-lg-start justify-content-lg-around w-100 mt-lg-3 mb-lg-3`}
                >
                    <div className="d-flex align-items-center mb-3 mb-lg-0 text-center">
                ...
                <div className={`d-flex align-items-center justify-content-center mt-auto mb-1`}>
                    ...
                    <button type="button" className={`btn btn-primary ms-3`} onClick={updateGame}>
                        Confirm
                    </button>
                </div>
            </div>
