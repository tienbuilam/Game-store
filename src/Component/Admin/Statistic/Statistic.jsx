...
function generateUniqueRGBAColors(n) {
    ...
    while (colorArray.length < n) {
        ...
        if (!colorArray.includes(rgbaColor)) colorArray.push(rgbaColor);
    }
...
const DrawOverall = () => {
    ...
    useEffect(() => {
        ...
        axios
            .post(`http://${domain}/admin/getOverall`, formData)
            .then((res) => {
                if (res.data.length) {
                    for (let i = 0; i < res.data.length; i++) res.data[i].value = parseInt(res.data[i].value);
                    setData(res.data);
                } else setData([]);
            })
        ...
        <div className="w-100 mb-5" style={{ maxHeight: data.length ? '500px' : '50px', maxWidth: '800px' }}>
            <fieldset className="mx-auto mt-3 d-flex align-items-center justify-content-sm-center justify-content-around">
                <div className="me-sm-5 me-2 text-center">
                    <label htmlFor="daily" className={`me-2 ${styles.fonts}`}>
                        Daily
                    </label>
                    <input
                        type="radio"
                ...
                <div className="text-center">
                    <label htmlFor="weekly" className={`me-2 ${styles.fonts}`}>
                        Weekly
                    </label>
                    <input
                        type="radio"
                ...
                <div className="ms-sm-5 ms-2 text-center">
                    <label htmlFor="monthly" className={`me-2 ${styles.fonts}`}>
                        Monthly
                    </label>
                    <input
                        type="radio"
                ...
                <div className="ms-sm-5 ms-2 text-center">
                    <label htmlFor="annually" className={`me-2 ${styles.fonts}`}>
                        Annually
                    </label>
                    <input
                        type="radio"
            ...
            <ResponsiveContainer width="100%" height="100%">
                <PieChart width="100%" height="100%">
                    ...
                    <Legend />
                    <Tooltip
                        wrapperStyle={{ background: 'rgba(255, 255, 255, 0.5)', padding: '5px', borderRadius: '5px' }}
    ...
    return (
        <div className={`d-flex align-items-center col-${12 / props.length} mb-3`}>
            <input
                ...
                className={`${styles.hover}`}
                type="radio"
                checked={props.category === props.InitCategory}
                onChange={() => props.setCategory(props.category)}
            ></input>
...
const Line = (props) => {
    ...
    return <div className="row mx-auto" ref={target} style={{ width: '98%' }}></div>;
};
...
const DrawGameTable = ({ div, Navigate }) => {
    ...
    useEffect(() => {
        ...
        axios
            .post(`http://${domain}/admin/getCategories`, formData)
            .then((res) => {
        ...
        axios
            .post(`http://${domain}/admin/getStats`, formData1)
            .then((res) => {
                if (res.data.length) {
                    const temp = [];
    ...
    return (
        <>
            <div className="mx-auto d-flex align-items-center mt-3">
                <p style={{ marginBottom: '0', marginRight: '10px' }} className={`${styles.fonts}`}>
                    Category:{' '}
                </p>
                <Button variant="primary" size="sm" onClick={() => setShowPopup(true)}>
                    Choose
                </Button>
            </div>
            <fieldset className="mx-auto mt-3 d-flex align-items-center">
                <div className="me-sm-5 me-2 text-center">
                    <label htmlFor="daily1" className={`me-2 ${styles.fonts}`}>
                        Daily
                    </label>
                    <input
                        type="radio"
                ...
                <div className="text-center">
                    <label htmlFor="weekly1" className={`me-2 ${styles.fonts}`}>
                        Weekly
                    </label>
                    <input
                        type="radio"
                ...
                <div className="ms-sm-5 ms-2 text-center">
                    <label htmlFor="monthly1" className={`me-2 ${styles.fonts}`}>
                        Monthly
                    </label>
                    <input
                        type="radio"
                ...
                <div className="ms-sm-5 ms-2 text-center">
                    <label htmlFor="annually1" className={`me-2 ${styles.fonts}`}>
                        Annually
                    </label>
                    <input
                        type="radio"
            ...
            <input
                ...
                className={`border border-dark rounded ${styles.fonts} mt-3 mb-3`}
                onChange={(e) => {
                    clearTimeout(timer1);
            ...
            <div
                className="overflow-auto hideBrowserScrollbar mb-2"
                style={{ minHeight: tableContent.length ? '400px' : '50px', width: '95%' }}
            >
            ...
            <Modal
                ...
                className={`reAdjustModel ${styles.modal} hideBrowserScrollbar`}
            ...
            >
                ...
                <Modal.Body className={`d-flex justify-content-center overflow-auto ${styles.modal_body}`}>
                    <fieldset className="w-100">{content}</fieldset>
                </Modal.Body>
            </Modal>
        </>
    );
...
const DrawCustomerTale = ({ Navigate }) => {
    ...
    useEffect(() => {
        axios
            .get(`http://${domain}/admin/getTopCustomers`)
            .then((res) => {
                if (res.data.length) {
                    const temp = [];
                    for (let i = 0; i < res.data.length; i++)
                        temp.push(
                            <tr
                                ...
                                onClick={() => Navigate(`/admin/customer-list/${res.data[i].id}`)}
                            >
    ...
    return (
        <div
            className="overflow-auto hideBrowserScrollbar mb-2"
            style={{ minHeight: tableContent.length ? '300px' : '50px', width: '95%' }}
        >
            <table className="table table-hover" style={{ borderCollapse: 'separate' }}>
                <thead style={{ position: 'sticky', top: '0', backgroundColor: '#BFBFBF' }}>
                    <tr>
