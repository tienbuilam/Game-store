...
const History = ({ name, code, date, price, method }) => {
    ...
    return (
        <tr>
            ...
            <td className="col-3 text-center">{formatCode(code)}</td>
...
function CustomerPersonalInfo() {
    ...
    useEffect(() => {
        axios.get(`http://${domain}/info`, { withCredentials: true }).then((res) => {
            setCustomer({
                name: res.data.name,
            ...
            if (isRefValid(imageRef))
                imageRef.current.src =
                    res.data.image === null ? person : `http://${domain}/model/data/customers/${res.data.image}`;
        });
    }, [renderTrigger]);

    const getHistory = () => {
        axios
            .get(`http://${domain}/info/history`, { withCredentials: true })
            .then((res) => {
                if (res.data.length !== 0) if (isRefValid(div2Height)) div2Height.current.style.display = 'block';
                ...
                if (isRefNotValid(target) && isRefValid(history)) target.current = ReactDOM.createRoot(history.current);
    ...
    const changeInfo = () => {
        if (isRefValid(customer_name)) customer_name.current.style.display = 'none';
        if (isRefValid(customer_email)) customer_email.current.style.display = 'none';
        if (isRefValid(customer_phone)) customer_phone.current.style.display = 'none';
        if (isRefValid(customer_dob)) customer_dob.current.style.display = 'none';
        if (isRefValid(edit)) edit.current.style.display = 'none';
        if (isRefValid(update)) update.current.style.display = 'flex';
        ...
        if (isRefValid(image_input)) image_input.current.style.setProperty('display', 'block', 'important');
    ...
    const cancelUpdate = () => {
        if (isRefValid(customer_name)) customer_name.current.style.display = 'inline';
        if (isRefValid(customer_email)) customer_email.current.style.display = 'inline';
        if (isRefValid(customer_phone)) customer_phone.current.style.display = 'inline';
        if (isRefValid(customer_dob)) customer_dob.current.style.display = 'inline';
        if (isRefValid(edit)) edit.current.style.display = 'inline-block';
        if (isRefValid(update)) update.current.style.display = 'none';
        if (isRefValid(customer_name_input)) customer_name_input.current.style.display = 'none';
        if (isRefValid(customer_email_input)) customer_email_input.current.style.display = 'none';
        if (isRefValid(customer_phone_input)) customer_phone_input.current.style.display = 'none';
        if (isRefValid(customer_dob_input)) customer_dob_input.current.style.display = 'none';
        if (isRefValid(image_input)) image_input.current.style.setProperty('display', 'none', 'important');
        if (isRefValid(password_input)) password_input.current.style.display = 'none';
        if (isRefValid(repassword_input)) repassword_input.current.style.display = 'none';
        setWrong(false);
        setIsFuture(false);
    };
    ...
    const confirmChange = (e) => {
        if (e !== undefined) {
            e.preventDefault();
            if (pass !== repass) setWrong(true);
            else if (
                isRefValid(customer_dob_input) &&
                customer_dob_input.current.value !== '' &&
                new Date(customer_dob_input.current.value) > new Date()
            )
                setIsFuture(true);
            else {
                setWrong(false);
                ...
                formData.append(
                    'phone',
                    isRefValid(customer_phone_input)
                        ? customer_phone_input.current.value === ''
                            ? null
                            : customer_phone_input.current.value
                        : null,
                );
    ...
    const changeImage = (e) => {
        if (e.target.files.length === 0) {
            setImage(null);
            if (isRefValid(imageRef)) imageRef.current.src = customer.image;
        } else {
            ...
            reader.onload = () => {
                ...
                if (isRefValid(imageRef)) imageRef.current.src = url;
            };
        }
    };
    ...
    return (
        <form
            className="h-100 w-100 overflow-auto d-flex align-items-center justify-content-center"
        ...
        >
            <div className="w-100 h-100 d-flex align-items-center">
                <div className={`flex-grow-1 w-100 overflow-auto mb-3 mt-3`}>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="text-center" style={{ marginBottom: '16px' }}>
                                ...
                                <input
                                    title="Your phone number must not contain alphabetical character(s)"
                                    type="text"
                                    className={`${styles.update} `}
                            ...
                            <div
                                className={`${styles.update} text-center w-100`}
                                ref={password_input}
                            ...
                            <div className="d-flex align-items-center" style={{ color: 'red' }}>
                                ...
                                {wrong && <p className={`${styles.p}`}>Passwords are not matched!</p>}
                                {isFuture && (
                                    <AiOutlineCloseCircle
                                        style={{
                                            marginRight: '5px',
                                ...
                                {isFuture && (
                                    <p className={`${styles.p} text-center`}>Your birthdate can not be the future!</p>
                                )}
                            </div>
                            <button type="button" className={`btn btn-sm btn-primary`} onClick={changeInfo} ref={edit}>
                                Edit
                            </button>
                            <div
                                ...
                                ref={update}
                            >
                                <button type="button" className={`btn btn-sm btn-danger`} onClick={cancelUpdate}>
                                    Cancel
                                </button>
                                <button
                                    type="button"
                    ...
                    <div
                        className={`w-100 mt-2 mb-4 overflow-auto ${styles.table} flex-grow-1`}
                        ref={div2Height}
